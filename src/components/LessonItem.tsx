...
const LessonItem: React.FC<LessonItemProps> = ({ lesson, isOpen }) => {
  return (
    <details
      open={isOpen}
      className="group bg-accent/[0.03] border border-accent/10 rounded-xl overflow-hidden mb-3 transition-all hover:border-accent/30"
    >
      <summary className="list-none cursor-pointer p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1 text-left">
          ...
          <p className="text-sm text-ink/60 leading-relaxed">{lesson.subtitle}</p>
        </div>
        <div className="flex flex-wrap gap-2 items-center">
          {lesson.tags.map((tag) => (
            <span
              key={tag}
              className="px-2 py-0.5 text-[0.7rem] font-mono rounded-full border border-accent/20 bg-accent/5 text-accent/80 font-bold"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="p-4 pt-0 space-y-4">
        {lesson.objectives && (
          <div className="bg-accent/5 border border-accent/10 rounded-xl overflow-hidden">
            <div className="bg-accent/10 px-3 py-2 border-b border-accent/10 flex justify-between items-center">
              <span className="text-xs font-black uppercase tracking-wider text-accent/60">
                Objectives
              </span>
              ...
              {lesson.exercise.code && (
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] font-mono text-accent/40">
                      {lesson.exercise.code.title}
                    </span>
        ...
        {lesson.criteria && (
          <div className="p-4 border border-dashed border-accent/30 bg-accent/5 rounded-xl">
            <span className="text-xs font-black text-accent mb-2 block uppercase tracking-widest">
              Acceptance Protocol
            </span>
